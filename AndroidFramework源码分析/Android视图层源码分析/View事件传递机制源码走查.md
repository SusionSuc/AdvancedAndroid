...
>上一篇文章我们看了[触摸事件的产生 -> Activity.dispatchTouchEvent()的整个过程](Android触摸事件全过程分析.md)。本文就从`Activity.dispatchTouchEvent()`来看一下触摸事件是如何在`View`中进行分发的。触摸事件分发的源码还是比较简单的(代码量比较少)。
...
public boolean dispatchTouchEvent(MotionEvent ev) {
    ...
    if (getWindow().superDispatchTouchEvent(ev)) { //如果Window可以处理，则事件处理完成
        return true;
    }
...
```
    ...
    if (actionMasked == MotionEvent.ACTION_DOWN || mFirstTouchTarget != null) { //在第一次派发事件时,mFirstTouchTarget必定为null
        //在第一次事件派发(DOWN事件)过程中子View可以请求父View不要对触摸事件做拦截。 但是必须是父View在onInterceptTouchEvent中返回了false。否则子View是无法请求父View不要去拦截事件的
...
```
    ...
    for (int i = childrenCount - 1; i >= 0; i--) { 
        ...
        //对于第一次(ACTION_DOWN)寻找事件处理者，这个方法返回null。 
        //如果进行了ACTION_DOWN事件的派发，那么这个方法会直接确定这个child是否是能处理事件的那个child(touchTarget)，如果是，则直接把事件派发给他
        ...
        if (dispatchTransformedTouchEvent(ev, false, child, idBitsToAssign)) {  //这个方法会调用子View的dispatchTouchEvent，看这个child是否能处理事件
            //子View可以处理这次事件
            ...
            //这个touchTarget就是 mFirstTouchTarget
